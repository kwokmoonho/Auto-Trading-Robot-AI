...
class FScore:
    ...
    def __init__(self, tickers):
        ...
        self.financial_dir_cy = {} #directory to store current year's information
        self.financial_dir_py = {} #directory to store last year's information
        self.financial_dir_py2 = {} #directory to store last to last year's information
    ...
    def dataHandling(self):
        for ticker in self.tickers:
            ...
            #getting balance sheet data from yahoo finance for the given ticker
                ...
                self.financial_dir_cy[ticker] = temp_dir
                self.financial_dir_py[ticker] = temp_dir2
                self.financial_dir_py2[ticker] = temp_dir3
            except:
                print("Problem scraping data for ",ticker)
        
        
        #storing information in pandas dataframe
        combined_financials_cy = pd.DataFrame(self.financial_dir_cy)
        #combined_financials_cy.dropna(axis=1,inplace=True) #dropping columns with NaN values
        combined_financials_py = pd.DataFrame(self.financial_dir_py)
        #combined_financials_py.dropna(axis=1,inplace=True)
        combined_financials_py2 = pd.DataFrame(self.financial_dir_py2)
